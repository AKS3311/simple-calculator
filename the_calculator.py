"""  
NOTE Please read the comments first before doing anything and thank you

-Install sympy simply by writing "pip install sympy"
-Iirst class for help functions that get the job done easier second class for the calculator functions third class for the main menu third class for running the hall program
-If you wanna add any calculator functions like formulas put them in the second class and then put the name of the function in the show_main_menu in the third class

Would love to see more formula functions in this god damn calculator :>
"""
...
Enter your choice (1 or 2): """
                ...
                while True:
                    cubic_choice1 = input(cubic_text).strip()  # Corrected here
                    if cubic_choice1 == '1':
                        self.help_func.clear_screen()
                        General_Form_cubin_text = """
...
If the equation has real roots, they will be displayed.
                        """
                        self.help_func.text_helper(General_Form_cubin_text)


                        def deg3(a, b, c, d, g):  # Using Newton-Raphson method
    ...
    f(x) = a(x-r1)(x-r2)(x-r3)
            """
                        ...
                        while True:
                            ...
                            # Calculate coefficients for the expanded cubic polynomial
                            A = a  # Coefficient of x^3
                            B = -a * (r1 + r2 + r3)  # Coefficient of x^2
                            C = a * (r1 * r2 + r2 * r3 + r3 * r1)  # Coefficient of x
                            D = -a * r1 * r2 * r3  # Constant term

                            # Prepare coefficient formatting
                            coeffs = [A, B, C, D]
                            coeffs_formatted = [f"{int(coeff)}" if coeff == int(coeff) else f"{coeff}" for coeff in coeffs]

                            # Create the polynomial string, avoiding signs for zero
                            polynomial_parts = []
                            for index, coeff in enumerate(coeffs_formatted):
                                power = len(coeffs_formatted) - 1 - index  # Determine the power of x
                                if coeff != "0":  # Skip zero coefficients
                                    if power == 0:
                                        polynomial_parts.append(f"{coeff}")
                                    elif power == 1:
                                        polynomial_parts.append(f"{coeff}x")
                                    else:
                                        polynomial_parts.append(f"{coeff}x^{power}")

                            polynomial_string = " + ".join(polynomial_parts).replace("+ -", "- ")
